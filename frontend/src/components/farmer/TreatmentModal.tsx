...
import { ArrowLeft, Pill, Calendar, Clock, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export const TreatmentModal: React.FC<TreatmentModalProps> = ({ animals, onBack, onSuccess }) => {
  const { t } = useLanguage();

  const [selectedAnimalId, setSelectedAnimalId] = useState<string>(animals[0]?.id || 'a102');
  ...
  const [indication, setIndication] = useState<string>('Mastitis (थन में सूजन)');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resData.status === 'success') {
        const safeDateFormatted = new Date(resData.data.safeDateISO).toLocaleDateString('hi-IN', {
          day: 'numeric',
          month: 'long',
          year: 'numeric',
        });
        ...
        setResultMessage({
          success: true,
          safeDate: safeDateFormatted,
          withdrawalDays: resData.data.ruleApplied.withdrawalDays,
          msg: resData.data.messageHi,
        });
        ...
        setResultMessage({
          success: false,
          msg: resData.message || 'Error calculating withdrawal period',
        });
      }
    } catch {
      setResultMessage({
        success: false,
        msg: 'Server error. Please verify backend API is running.',
      });
    } finally {
          ...
          <div className="p-6 bg-[#E8F5E9] border-2 border-[#1B5E20] rounded-2xl space-y-4 text-[#1B5E20]">
            ...
            <div className="bg-white p-4 rounded-xl border border-[#1B5E20]/30 space-y-2 text-xs font-bold">
              <div className="flex justify-between items-center">
                ...
                <span className="font-black text-[#B78103]">{resultMessage.withdrawalDays} दिन (Days)</span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 text-xs font-bold text-gray-800">
              ...
              >
                {animals.map((a) => (
                  <option key={a.id} value={a.id}>
                    {a.animal_code} ({a.species === 'cow' ? 'गाय' : 'भैंस'} - {a.breed})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="उदा: Mastitis, Fever, Pneumonia"
                className="w-full bg-[#FFFDF5] border-2 border-gray-300 rounded-2xl px-4 py-3 text-gray-900 focus:border-[#1B5E20] focus:outline-none"
              />
            </div>
