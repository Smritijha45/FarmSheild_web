...
router.get('/withdrawals', (_req, res) => {
    const now = new Date();
    const activeWithdrawals = dbService_1.db.withdrawals.map((w) => {
        const animal = dbService_1.db.animals.find((a) => a.id === w.animal_id);
        const treatment = dbService_1.db.treatments.find((t) => t.id === w.treatment_id);
        const medicine = dbService_1.db.medicines.find((m) => m.id === treatment?.medicine_id);
